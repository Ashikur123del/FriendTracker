...
const FriendsDetails = () => {
  ...
  if (!friend)
    return <div className="p-20 text-center font-bold">Friend Not Found</div>;
  ...
  const handleAction = (type) => {
    ...
    toast(`${type} data synced for ${name}`);
  };
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 text-center relative overflow-hidden">
            <div
              className={`absolute top-0 left-0 w-full h-2 ${statusMap[status?.toLowerCase()] || "bg-slate-300"}`}
            ></div>
            <img
              src={picture}
              alt={name}
              className="w-28 h-28 rounded-full mx-auto mb-4 object-cover border-4 border-slate-50 shadow-sm"
            />
            ...
            <p className="mt-8 text-slate-500 text-sm leading-relaxed italic">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 text-center">
              ...
              <p className="text-slate-400 text-xs font-bold uppercase mt-3">
                Next Due
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center">
            <div>
              ...
              <p className="text-slate-500 text-sm">
                Connect every{" "}
                <span className="font-bold text-slate-900">{goal} days</span>
              </p>
            </div>
          ...
          <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
            ...
            <div className="grid grid-cols-3 gap-4">
              <button
                ...
                className="flex flex-col items-center gap-3 p-6 rounded-2xl bg-blue-50 transition-all border border-transparent hover:border-blue-100 group"
              >
                <LuPhone className="text-2xl text-slate-600 group-hover:text-blue-600" />
                <span className="font-bold text-[10px] uppercase tracking-wider text-slate-400 group-hover:text-blue-600">
                  Call
                </span>
              </button>
              <button
                onClick={() => handleAction("Text")}
                className="flex flex-col items-center gap-3 p-6 rounded-2xl bg-emerald-50 transition-all border border-transparent hover:border-emerald-100 group"
              >
                <LuMessageSquare className="text-2xl text-slate-600 group-hover:text-emerald-600" />
                <span className="font-bold text-[10px] uppercase tracking-wider text-slate-400 group-hover:text-emerald-600">
                  Text
                </span>
              </button>
              <button
                onClick={() => handleAction("Video")}
                className="flex flex-col items-center gap-3  p-6 rounded-2xl bg-purple-50 transition-all border border-transparent hover:border-purple-100 group"
              >
                <LuVideo className="text-2xl text-slate-600 group-hover:text-purple-600" />
                <span className="font-bold text-[10px] uppercase tracking-wider text-slate-400 group-hover:text-purple-600">
                  Video
                </span>
              </button>
            </div>
